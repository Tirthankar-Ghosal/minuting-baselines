...
class BertParent(object):
    ...
    def __init__(self, model: str, vector_size: int=None):
        self.model = BertModel.from_pretrained(model).to("cuda")
    ...
    def tokenize_input(self, text: str) -> torch.tensor:
        ...
        return torch.tensor([indexed_tokens]).to("cuda")
    ...
    def create_matrix(self, content: List[str], hidden: int=-2, reduce_option: str = 'mean') -> ndarray:
        ...
        for i, t in tqdm(enumerate(content)):
            train_vec[i] = self.extract_embeddings(t, hidden=hidden, reduce_option=reduce_option).cpu().data.numpy()
        return train_vec
